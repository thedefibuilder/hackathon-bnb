...
type TAuditSectionProperties = {
  activeChainName: string;
  audit: TVulnerability[];
};

export default function AuditSection({ activeChainName, audit }: TAuditSectionProperties) {
      ...
      <SectionHeader>
        <div className='flex flex-col'>
          ...
          <h4 className='text-base font-medium text-muted-foreground md:text-lg'>
            Get to know how&apos;s your {activeChainName} Smart Contract
          </h4>
        </div>

        {audit.length > 0 && (
          <Suspense fallback={<Skeleton className='h-10 md:w-[8.5rem]' />}>
